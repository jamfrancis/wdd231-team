...
// DOM elements
let searchForm, searchInput, errorContainer, favoriteButton, unitToggle, fahrenheitUnit, celsiusUnit, recentSearchesContainer, recentSearchesList;
...
function updateHourlyForecast(data) {
    ...
    // If we don't have enough hours from today, get remaining hours from tomorrow
    if (next11Hours.length < 11) {
        const remainingHours = 11 - next11Hours.length;
        const tomorrowHours = data.forecast.forecastday[1].hour
            .slice(0, remainingHours);
        next11Hours.push(...tomorrowHours);
    }

    // Add current hour as "Now"
    const currentHourElement = document.createElement('div');
    currentHourElement.innerHTML = hourlyForecastTemplate(currentHour, getTemperatureUnit(), timezone, true);
    hourlyForecastContainer.appendChild(currentHourElement.firstElementChild);

    // Add each future hour to the container
    next11Hours.forEach(hour => {
        const hourElement = document.createElement('div');
        hourElement.innerHTML = hourlyForecastTemplate(hour, getTemperatureUnit(), timezone);
        hourlyForecastContainer.appendChild(hourElement.firstElementChild);
    });
}
